    ...
    if visible_field[starting_position[0], starting_position[1]] == 0:
        ...
        while len(selected_cell_list) != 0:
            for cell in selected_cell_list:
                ...
                selected_cell_list.remove((cell[0], cell[1]))
                if bomb_field[cell[0], cell[1]] == 0:
...
def update_cells_around_selected_cell(selected_cell, bomb_field, visible_field):
    if bomb_field[selected_cell[0], selected_cell[1]] != 9:
        new_cells_coordinates = []
        visible_field[selected_cell[0], selected_cell[1]] = bomb_field[selected_cell[0], selected_cell[1]]
        for i in range(selected_cell[0] - 1, selected_cell[0] + 2):
            for j in range(selected_cell[1] - 1, selected_cell[1] + 2):
                if i >= 0 and j >= 0 and i < number_of_rows and j < number_of_columns:
                    if bomb_field[i, j] == 0:
                        visible_field[i, j] = bomb_field[i, j]
